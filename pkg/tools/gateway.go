...
func (g *ToolGateway) NormalizeCall(call types.ToolCall, defs []types.ToolDef, clientDialect string) types.ToolCall {
	...
	if strings.HasPrefix(strings.ToLower(call.Name), "mcp__") {
		...
		for _, d := range defs {
			if strings.EqualFold(d.Name, "call_mcp_tool") {
				hasCallMCP = true
				callMCPDef = d
				break
			}
...
func (g *ToolGateway) findMatchingToolDef(incomingName string, defs []types.ToolDef, clientDialect string) (types.ToolDef, bool) {
	...
	// Fallback MCP tool matching (e.g. incoming is "github_create_issue", def is "mcp__github__create_issue")
	cleanIncoming := strings.TrimPrefix(lowerIncoming, "mcp__")
	cleanIncoming = strings.TrimPrefix(cleanIncoming, "mcp_")
	cleanIncoming = strings.ReplaceAll(strings.ReplaceAll(cleanIncoming, "__", "_"), "-", "_")
	for _, d := range defs {
		dClean := strings.TrimPrefix(strings.ToLower(d.Name), "mcp__")
		dClean = strings.TrimPrefix(dClean, "mcp_")
		dNorm := strings.ReplaceAll(strings.ReplaceAll(dClean, "__", "_"), "-", "_")
		if dNorm == cleanIncoming || strings.HasSuffix(dNorm, "_"+cleanIncoming) || strings.HasSuffix(cleanIncoming, "_"+dNorm) {
			return d, true
		}
	}
